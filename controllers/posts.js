...
exports.viewPostDetails = async(req, res) => {
    ...
    try{
        ...
        const post = await Post.findById(post_id).populate("place", "name address google_types types").populate("user", "name");
        if(!post){
            errors.general = "Post not found";
            return res.status(404).json({
                success: false,
...
exports.updatePostLocation = async(req, res) => {
    ...
    try{
        ...
        const newPost = await Post.findById(post._id).populate("place", "name address google_types types").populate("user", "name");;
