  ...
  try {
    ...
    if (place.duplicate_place_id.length > 0) {
      await Place.updateMany(
        { _id: { $in: place.duplicate_place_id } },
        {
  ...
  try {
    ...
    const results = await Place.find({
      $or: [
        ...
        { alias: { $regex: `${trimedPlace}`, $options: "i" } },
      ],
    })
      .where("duplicate")
      .ne(true)
...
exports.setOriginalPlace = async (req, res) => {
  ...
  try {
    ...
    if (currentPlace.original_place_id) {
      const oldOriginalPlace = await Place.findById(
        currentPlace.original_place_id
      );
      if (
        oldOriginalPlace &&
        oldOriginalPlace.duplicate_place_id.includes(currentPlace._id)
      ) {
        const index = oldOriginalPlace.duplicate_place_id.indexOf(
          currentPlace._id
        );
    ...
    await Post.updateMany(
      ...
      { original_place: originalPlace._id }
    );
...
exports.deleteOriginalPlace = async (req, res) => {
  ...
  try {
    ...
    if (originalPlace.duplicate_place_id.includes(duplicatePlace._id)) {
      const index = originalPlace.duplicate_place_id.indexOf(
        duplicatePlace._id
      );
      ...
      const newP = await Post.updateMany(
        {
          $and: [
            { place: duplicatePlace._id },
            { original_place: originalPlace._id },
          ],
        },
        { $unset: { original_place: 1 } }
      );
    }
...
exports.setOriginalDisplayName = async (req, res) => {
  try {
    const places = await Place.find({ duplicate: true }).populate(
      "original_place_id"
    );
    ...
    for (let doc of places) {
      Place.updateOne(
        { _id: doc._id },
        { $set: { display_name: doc.original_place_id.name } }
      );
    }
